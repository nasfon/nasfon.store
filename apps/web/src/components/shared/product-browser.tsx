...
import { ProgressLoader, DotsLoader } from "@/components/ui/loader";
...
export function ProductBrowser({ initialData, categories: initialCategories }: ProductBrowserProps) {
  const [search, setSearch] = useState("");
  const [searchInput, setSearchInput] = useState("");
  const [categoryId, setCategoryId] = useState("");
  const [sort, setSort] = useState("newest");

  const isDefaultView = !search && !categoryId && sort === "newest";

  const { data, isLoading, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteProducts(
    {
      limit: 12,
    ...
    isDefaultView
      ? { initialData: { pages: [initialData], pageParams: [initialData.pagination.page] } }
      : undefined
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className={`grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 ${isFetching && !isLoading ? "mt-4" : "mt-6"}`}>
        {isLoading ? (
