...
export async function sendOrderConfirmation(params: {
  ...
  const paymentHtml = params.payment.bank_name
    ? `
    <tr><td style="padding:16px 0 0">
      <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;padding:16px">
        <p style="margin:0 0 12px;font-size:14px;font-weight:600;color:#0369a1">Bank Transfer Instructions</p>
        <table width="100%">
          <tr><td style="font-size:13px;color:#666;padding:4px 0">Bank:</td><td style="font-size:13px;color:#333;font-weight:500;padding:4px 0;text-align:right">${params.payment.bank_name}</td></tr>
          <tr><td style="font-size:13px;color:#666;padding:4px 0">Account Number:</td><td style="font-size:13px;color:#333;font-weight:600;padding:4px 0;text-align:right">${params.payment.account_number}</td></tr>
          <tr><td style="font-size:13px;color:#666;padding:4px 0">Account Name:</td><td style="font-size:13px;color:#333;font-weight:500;padding:4px 0;text-align:right">${params.payment.account_name}</td></tr>
        </table>
      </div>
    </td></tr>`
    : "";
  ...
  const { error } = await resend.emails.send({
    ...
    html: layout(`
      <tr><td style="padding:24px 32px">
        ...
        <p style="margin:0 0 16px;font-size:14px;color:#666">Your order <strong style="color:#333">${params.orderNumber}</strong> has been placed successfully.</p>
        <p style="margin:0 0 16px;font-size:14px;color:#666">Please complete your payment using the details below to confirm your order.</p>
        ...
        ${params.payment.expires_at ? `<p style="margin:16px 0 0;font-size:12px;color:#ef4444">This payment reference expires on ${new Date(params.payment.expires_at).toLocaleString()}.</p>` : ""}

        <p style="margin:16px 0 0;font-size:13px;color:#666">Once payment is confirmed, we'll process your order and notify you.</p>
      </td></tr>
    `),
  });
